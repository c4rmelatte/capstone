...
const CustomTextInput: React.FC<CustomTextInputProps> = ({ 
  ...
  return (
    <View className="justify-center mb-3">
      {inputName ? (
        <Text className="text-gray-500 font-bold mb-1 ml-4">
          {inputName}
        </Text>
      ) : null}
      
      <View className="w-[300px] h-[45px] bg-white border-2 border-black rounded-[100px] flex-row items-center px-5">
        {iconName && (
          <Feather name={iconName} size={20} color="#D1D1D1" />
        )}
        
        <TextInput
          className="flex-1 px-3  text-black font-bold"
