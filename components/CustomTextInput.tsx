...
import { Text, TextInput, TextInputProps, TouchableOpacity, View, Dimensions } from "react-native";
...
const CustomTextInput: React.FC<CustomTextInputProps> = ({ 
  ...
  const fontSize = SCREEN_WIDTH * 0.04; // text ~4% of screen width
  const labelFontSize = SCREEN_WIDTH * 0.035; // label slightly smaller

  return (
    <View style={{ justifyContent: 'center', marginBottom: 12 }}>
      {inputName ? (
        <Text style={{ color: '#6B7280', fontWeight: 'bold', marginBottom: 4, marginLeft: 8, fontSize: labelFontSize }}>
          {inputName}
        </Text>
      ) : null}
